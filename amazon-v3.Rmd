...
```{r}
set.seed(697)
pick_rows <- sample(nrow(proc_df), 30000)
df_sample <- proc_df[pick_rows, ]
heatmap(df_sample)
```
```{r}
library(gplots)
heatmap.2(df_sample,
          ...
          notecol = "black",
          labRow = NA, # do not show row labels
          labCol = NA, # do not show column labels
          key = TRUE, # show color key
          cexRow = 1.0, # set row label size
          cexCol = 1.0 # set column label size
)
```
